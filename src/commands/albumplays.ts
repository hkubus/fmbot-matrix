...
export async function run(
	...
	if (!albumInfo)
		return client.replyText(
			roomId,
			message.eventId,
			"couldn't fetch album info",
		);
	client.replyText(
		roomId,
		message.eventId,
		`${message.sender} has listened to \`${albumInfo.title}\` by ${albumInfo.artist} ${albumInfo.plays === "1" ? "once" : `${albumInfo.plays || 0} times`}`,
		`<a href="https://matrix.to/#/${message.sender}">${message.sender.split(":")[0]}</a> has listened to \`${albumInfo.title}\` by ${albumInfo.artist} ${albumInfo.plays === "1" ? "once" : `${albumInfo.plays || 0} times`}`,
	);
}
